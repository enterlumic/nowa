let detalle = {
    ...
    fn_set_import_detalle: function () {
        $("#form_import_detalle").validate({
            submitHandler: function (form) {
                let get_form = document.getElementById("form_import_detalle");
                let postData = new FormData(get_form);

                let element_by_id= 'form_import_detalle';
                let message=  'Cargando...' ;
                let $loading= LibreriaGeneral.f_cargando(element_by_id, message);

                $.ajax({
                    url: "set_import_detalle",
                    data: postData,
                    cache: false,
                    processData: false,
                    contentType: false,
            ...
            , messages: {
                vCampo1_detalle: {
                    minlength: "Mensaje personalizado vCampo1_detalle"
                }
              }
        });
    },

    fn_modalShowdetalle: function () {
        $('#modalFormIUdetalle').on('shown.bs.modal', function (e) {
            $('#vCampo1_detalle', e.target).focus();
        });

        $('#modalImportFormdetalle').on('shown.bs.modal', function (e) {
