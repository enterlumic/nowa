var Perfil_usuario = {
    ...
    init: function () {
        ...
        Perfil_usuario.set_Perfil_usuario();

        // Funciones para eventos
        Perfil_usuario.upload();
        document.querySelector("#profile-img-file-input").addEventListener("change", function() {
            var e = document.querySelector(".user-profile-image"),
                photo = document.querySelector(".profile-img-file-input").files[0],
                n = new FileReader;                
                ...
                fetch('uploadFiles', {method: "POST", body: formData})
                .then( response => response.json() )
                .then( response => $('.rounded-circle').attr('src', response['data']['vc_path']) );
                
            n.addEventListener("load", function() {
                e.src = n.result;
            }, !1), photo && n.readAsDataURL(photo)
        });
    },

    set_Perfil_usuario: function () {
        $("#form_perfil_usuario").validate({
            submitHandler: function (form) {
                var get_form = document.getElementById("form_perfil_usuario");
                var postData = new FormData(get_form);

                let element_by_id= 'form_perfil_usuario';
                let message=  'Actualizando...' ;
                let $loading= LibreriaGeneral.f_cargando(element_by_id, message);

                $.ajax({
                    url: "set_perfil_usuario",
                    data: postData,
                    cache: false,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (response) {
                        ...
                        if (json["b_status"]) {
                            // Buscar si existe alguna clase para actulizar el nombre al momento de actualizar...
                            if ( $(".user-name-text").length )
                                $(".user-name-text").html( $("#name").val() );
            ...
            upload: {
                ...
                beforeSend: function(item, listEl, parentEl, newInputEl, inputEl) {
                    ...
                    if (item.editor && item.editor._blob) {
                        ...
                        item.upload.formData.append('_token', $('meta[name="csrf-token"]').attr('content'));
                    }
