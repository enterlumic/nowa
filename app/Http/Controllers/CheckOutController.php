    ...
    public function index(Request $request)
    {
        ...
        if (isset($_REQUEST['id']) && $id == 'carrito'){
            ...
            $productos = DB::table('carrito as c')
                ...
                ->select('p.id', 'c.cantidad', 'p.titulo', 'p.tiempo_trabajador', 'pf.foto_url', DB::raw("CAST(REPLACE(p.precio, ',', '') AS UNSIGNED) as precio"), 'p.marca')
            ...
            $productos = DB::table('productos as p')
                ...
                ->select('p.id', DB::raw( '1 AS cantidad' ), 'p.titulo', 'p.tiempo_trabajador', 'pf.foto_url', DB::raw("CAST(REPLACE(p.precio, ',', '') AS UNSIGNED) as precio"), 'p.marca')
                ->get();
        }
